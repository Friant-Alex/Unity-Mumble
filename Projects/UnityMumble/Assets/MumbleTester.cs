...
public class MumbleTester : MonoBehaviour {
	...
	void Update () {
        ...
        {
            PostWithDigestAuth("http://127.0.0.1:8080/servers/1/user/1/mute");
        }
        if (Input.GetKeyDown(KeyCode.N))
        {
            PostWithDigestAuth("http://127.0.0.1:8080/servers/1/user/1/unmute");
        }
        ...
        {
            DeleteWithDigestAuth("http://127.0.0.1:8080/servers/1/channels/1");
        }
    ...
    {
        var serverInfo = GetWithDigestAuth("http://127.0.0.1:8080/servers/");
        yield return new WaitUntil(() => serverInfo.IsCompleted);
        Debug.Log(serverInfo.Result);

        var serverLogs = GetWithDigestAuth("http://127.0.0.1:8080/servers/1/logs");
        yield return new WaitUntil(() => serverLogs.IsCompleted);
        Debug.Log(serverLogs.Result);

        var serverConfig = GetWithDigestAuth("http://127.0.0.1:8080/servers/1/conf");
        yield return new WaitUntil(() => serverConfig.IsCompleted);
        Debug.Log(serverConfig.Result);

        var serverStatus = GetWithDigestAuth("http://127.0.0.1:8080/stats/");
        yield return new WaitUntil(() => serverStatus.IsCompleted);
        Debug.Log(serverStatus.Result);

        var serverUsers = GetWithDigestAuth("http://127.0.0.1:8080/servers/1/user");
        yield return new WaitUntil(() => serverUsers.IsCompleted);
        Debug.Log(serverUsers.Result);
    }
}
