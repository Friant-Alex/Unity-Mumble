...
namespace Mumble
{
    ...
    public class MumbleClient
    {
        ...
        public bool JoinChannel(string channelToJoin)
        {
            ...
            {
                KeyValuePair<string, string> formdata = new KeyValuePair<string, string>("name", channelToJoin);
                KeyValuePair<string, string> parentData = new KeyValuePair<string, string>("parent", "0");

                PostWithDigestAuth("http://127.0.0.1:8080/servers/1/channels", "admin", "password", formdata, parentData);
                return false;
            }
